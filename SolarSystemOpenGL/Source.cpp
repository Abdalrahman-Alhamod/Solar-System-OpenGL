...
#include <gl\glux.h>		// Header File For The Glaux Library
#include <gl\glut.h>		
#include <math.h>
#include "Point.h";
#include "Color.h";
...
void DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	glTranslatef(0, 0, TranslateZFromCenter);
	SolarSystem().controlCamera(keys);
...
BOOL CreateGLWindow(const char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	// Load Custome Icon
	HICON hIcon = (HICON)LoadImage(NULL, "SolarSystem.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
	if (hIcon)
	{
	...
	LPARAM	lParam)			// Additional Message Information
{
	...
	case WM_KEYDOWN:							// Is A Key Being Held Down?
	{
		...
		SolarSystem().decodeKeyboard(keys);
		if (wParam == 70)
		{
	...
	int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Solar System", 640, 480, 16, fullscreen))
	{
